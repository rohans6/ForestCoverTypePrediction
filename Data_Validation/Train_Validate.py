...
class Train_Data_Validater:
    ...
    def deleteExistingGoodFolder(self):
        ...
        """
        if os.path.isdir("Training_Validated_Files/Good/"):
            shutil.rmtree("Training_Validated_Files/Good/")

    def deleteExistingBadFolder(self):
        """
        This Function deletes a Bad Folder.
        """
        if os.path.isdir("Training_Validated_Files/Bad/"):
            shutil.rmtree("Training_Validated_Files/Bad/")

    def createFolders(self):
        """
        This Function created 2 Folders:- a. Good b. Bad
        """
        if not os.path.isdir("Training_Validated_Files/Good/"):
            os.mkdir("Training_Validated_Files/Good/")
        if not os.path.isdir("Training_Validated_Files/Bad/"):
            os.mkdir("Training_Validated_Files/Bad/")
            ...
            if re.match(self.pattern,file_name):
                ...
                if len(names[2])==LengthOfDateStampInFile:
                    if len(names[3])==LengthOfTimeStampInFile:
                        path=shutil.copy(self.path+"/"+file_name,"Training_Validated_Files/Good/")
                    else:
                       
                        path=shutil.copy(self.path+"/"+file_name,"Training_Validated_Files/Bad/")
                else:
    ...
    def validate_Column_Length(self,number_columns):
        ...
        """
        try:
            files=os.listdir("Training_Validated_Files/Good/")
            for file in files:
                df=pd.read_csv("Training_Validated_Files/Good/"+file)
                if not df.shape[1]==number_columns:
                    path=shutil.move("Training_Validated_Files/Good/"+file,"Training_Validated_Files/Bad/")    
                else:
                    pass
        except Exception as e:
            print(e)  
